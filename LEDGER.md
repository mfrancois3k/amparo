...
## 37 — Flipboard + Digg outreach pack, daily queue, and the routine that runs it

- **Date:** 2026-08-27 · **Commits:** `c562c76`, `653bd77`, `9803afe` (pushed)
- **Trigger:** operator asked for Flipboard magazine setup, then a Digg content prompt, then
  "make a routine for content creation for both platform... an optimize prompt that can do this
  for me" — explicitly clarifying it would run via Claude Code's own scheduled-tasks feature, not
  a script the operator triggers by hand.

### What shipped

`growth/outreach-pack.md` (3 Flipboard magazines, 27 seed links; 9 Digg submissions; 3 bilingual
Reels scripts) generated by `tools/build-outreach-pack.mjs`, which imports `LINES` from
`render-kyr-card.mjs` so Reel value-lines stay byte-identical to the WhatsApp cards and the arena.
`tools/outreach-queue.mjs` turns that static pack into a daily rotation: one Flipboard flip, one
Digg submission, one Reel script every 7 days, idempotent per calendar date, exhausted lists
reporting `done`+reason instead of silently wrapping. Wired to a new Claude Code scheduled task,
`amparo-daily-outreach` (daily 09:00 local), whose prompt drives the operator's real
(`claude-in-chrome`) Chrome session — see wargame 32 §15 for the full design and the guardrails
baked into that prompt (no login/CAPTCHA handling, no vote-driving, one item per platform per day).

### Two things declined, on the record

Both are the same category of decision, made twice: coordinated Digg upvoting (Wilson-lower-bound
gaming = vote brigading, any platform) and Reels reacting to a specific real trending clip (the
exact rule `tools/news-post.mjs` already enforces for daily posts — signal picks WHICH scenario,
never WHAT is said, no individual case ever enters a post). Declined in chat when first asked;
documented in `growth/outreach-pack.md` itself and in wargame 32 §15b so the reasoning survives
independent of this conversation.

### Bug found and fixed along the way

`render-kyr-card.mjs`'s `--selftest` block checked `process.argv.includes('--selftest')` at module
scope, un-gated by `isMain`. Any script importing it (transitively, via `build-outreach-pack.mjs`)
while `--selftest` was anywhere in `argv` got `process.exit()`-ed before running its own code —
`outreach-queue.mjs --selftest` silently ran render-kyr-card's 29 assertions and reported them as
success, never reaching its own 12. Caught because the reported check count didn't match what the
new selftest actually asserts. Fixed by gating on `isMain`, matching every other tool in the repo.

### Open / unresolved

- **Not yet verified end to end:** whether `claude-in-chrome`'s session persists Flipboard/Digg
  logins across separate scheduled-task runs on this machine. First real unattended run is
  2026-08-27 ~09:08 local. If it doesn't persist, the routine's own hard rule (never attempt login)
  means it degrades to "stopped, needs re-login" rather than doing anything unsafe — but this is
  design intent, not yet observed behavior.
- The scheduled-tasks tool recommends a manual "Run now" once to pre-approve the browser tools
  before the first unattended fire — operator has not yet done this as of this entry.
- Same open blockers as ledger 35/36: attorney sign-off (gates `PAYMENTS_LIVE`), Track F accounts
  (Search Console, beehiiv, sending-domain DNS, etc.), local CLINIC/promotora outreach.
