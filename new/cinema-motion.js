...
(() => {
  ...
  window.AmparoCinemaMath={clamp,frameAt,beatAt,sceneAt,blendAt};
  ...
  function mount(){
    ...
    let progress=0,width=1,height=1,rendition=mobile.matches?'mobile':'desktop',active=-1,tooTall=false;
    ...
    function paint(){
      ...
      const p=armed?progress:0;
      if(manifest)manifest.scenes.forEach((s,i)=>targets[i]=frameAt(clamp(p*3-i),s.count));
      ...
      shots.forEach((shot,i)=>{
        ...
        shot.style.transform='scale('+(1+.035*(1-clamp(p*3-i)))+')';
      });
    ...
    function disarm(){trigger?.kill();trigger=null;armed=false;root.classList.remove('is-scroll');progress=0;release();request();}
    function arm(){
    ...
    chapters.forEach((chapter,i)=>chapter.addEventListener('click',()=>{if(trigger)window.scrollTo({top:trigger.start+(trigger.end-trigger.start)*(i/3+.10),behavior:reduced.matches?'auto':'smooth'});},opts));
